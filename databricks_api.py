...
# Environment Setup
# ---------------------------------------------------------------------
INSTANCE = os.getenv("DATABRICKS_INSTANCE")  # Base URL; typically accounts host when using account-level APIs
WORKSPACE_INSTANCE = os.getenv("WORKSPACE_INSTANCE")  # Optional: workspace base URL for workspace-level SCIM fallback
TOKEN = os.getenv("DATABRICKS_TOKEN")  # Account-level token (or workspace if re-used)
WORKSPACE_TOKEN = os.getenv("WORKSPACE_TOKEN") or TOKEN  # Prefer dedicated workspace token
...
def _safe_request(method: str, url: str, retries: int = 3, headers: Optional[Dict[str, str]] = None, timeout: int = 30, **kwargs) -> Dict[str, Any]:
    """Perform a safe HTTP request with retries and consistent return shape."""
    delay = 2
    for attempt in range(1, retries + 1):
        try:
            resp = requests.request(method, url, headers=(headers or _auth_headers()), timeout=timeout, **kwargs)
            # --- Success path ---
            if resp.status_code in (200, 201):
                try:
                    return {"status_code": resp.status_code, "body": resp.json()}
                except Exception:
                    return {"status_code": resp.status_code, "body": {}, "message": "Empty body"}
...
def scim_list_users(filter_: Optional[str] = None) -> Dict[str, Any]:
    ...
    return _safe_request("GET", url, params=params, timeout=FAST_TIMEOUT_SECS, retries=FAST_RETRIES)
...
def scim_me_workspace() -> Dict[str, Any]:
    ...
    return _safe_request("GET", url, headers=_auth_headers(), timeout=FAST_TIMEOUT_SECS, retries=FAST_RETRIES)


def scim_create_user(payload: Dict[str, Any]) -> Dict[str, Any]:
    url = _url(f"/api/2.1/accounts/{ACCOUNT}/scim/v2/Users")
    return _safe_request("POST", url, json=payload, timeout=FAST_TIMEOUT_SECS, retries=FAST_RETRIES)
...
def scim_patch_user(user_id: str, patch_ops: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return _safe_request("PATCH", url, json=patch_ops, timeout=FAST_TIMEOUT_SECS, retries=FAST_RETRIES)
...
def create_project_folder(application_name):
    ...
    return _safe_request("POST", url, headers=_auth_headers(), json={"path": base_path}, timeout=FAST_TIMEOUT_SECS, retries=FAST_RETRIES), base_path
...
def get_policy_id(policy_name):
    ...
    api_response = _safe_request("GET", url, headers=_auth_headers(), timeout=FAST_TIMEOUT_SECS, retries=FAST_RETRIES)
    status_code = api_response.get("status_code")
    response_body = api_response.get("body", {})

    if status_code != 200:
        error_msg = api_response.get("error") or str(response_body)
        raise RuntimeError(f"Failed to retrieve cluster policies list. API Error: {error_msg}")
    ...
    if found_id:
        ...
        raise RuntimeError(f"Cluster policy '{policy_name}' not found in the workspace.")
...
def create_sql_warehouse(application_name, sql_wh_size, project_tags):
    ...
    custom_tags_list = [
        {"key": k, "value": v}
        for k, v in project_tags.items() if v
    ]
    formatted_tags = {
    ...
    if api_response.get("status_code") == 200:
        ...
        if new_wh_id:
            ...
            raise RuntimeError(f"SQL Warehouse creation succeeded but ID was missing from response body: {api_response.get('body')}")
    else:
        error_msg = api_response.get("error") or str(api_response.get("body"))
        raise RuntimeError(f"Failed to create SQL Warehouse '{wh_name}'. API Error: {error_msg}")
    

def get_object_id(object_name):
    """Retrives the Folder id"""
    url = _ws_url(f"/api/2.0/workspace/get-status")
    api_response = _safe_request("GET", url, headers=_auth_headers(), params={"path": f"{object_name}"}, timeout=FAST_TIMEOUT_SECS, retries=FAST_RETRIES)
    ...
    if status_code == 200:
        ...
        if found_id:
            ...
            raise RuntimeError(f"'object_id' was missing from response body for path: {object_name}")
    else:
        error_msg = api_response.get("error")
        if status_code == 404:
            raise RuntimeError(f"Workspace object not found at path '{object_name}' (Status 404).")
        else:
            raise RuntimeError(f"Failed to get object status for path '{object_name}'. API Error: {error_msg}")
...
def get_group_id(group_name):
    ...
    api_response = _safe_request("GET", url, headers=_auth_headers(), params={"filter": f"displayName eq \"{group_name}\""}, timeout=FAST_TIMEOUT_SECS, retries=FAST_RETRIES)
...
def create_project_groups(group_full_name):
    ...
    if group_id:
        print(f"Group '{group_full_name}' already exists. Using existing ID: {group_id}")
        return group_id
    ...
    if status_code in (200, 201):
        ...
        if new_group_id:
            ...
            raise RuntimeError(f"Group creation succeeded but ID was missing for {group_full_name}: {response_body}")    
    else:
        error_msg = api_response.get("error") or str(response_body)
        raise RuntimeError(f"Failed to create group '{group_full_name}'. API Error: {error_msg}")
...
def set_all_folder_permissions(folder_path, acl_list):
    ...
    if not request_obj_id:
        print(f"Error: Could not retrieve object ID for folder '{folder_path}'. Permissions skipped.")
        return
    ...
    api_response = _safe_request("PUT", url, headers=_auth_headers(), json=payload, timeout=FAST_TIMEOUT_SECS, retries=FAST_RETRIES)
    ...
    if status_code == 200:
        ...
        error_msg = api_response.get("error") or str(api_response.get("body"))
        raise RuntimeError(f"Failed to set folder permissions for '{folder_path}'. API Error: {error_msg}")
...
def set_all_cluster_permissions(cluster_id, acl_list):
    ...
    if status_code == 200:
        ...
        error_msg = api_response.get("error") or str(api_response.get("body"))
        raise RuntimeError(f"Failed to set cluster permissions for '{cluster_id}'. API Error: {error_msg}")
...
def set_all_sql_warehouse_permissions(warehouse_id, acl_list):
    ...
    if status_code == 200:
        ...
        error_msg = api_response.get("error") or str(api_response.get("body"))
        raise RuntimeError(f"Failed to set sql warehouse permissions for '{warehouse_id}'. API Error: {error_msg}")
...
def get_user_id(user_email):
    ...
    api_response = _safe_request("GET", url, headers=_auth_headers(), params=params, timeout=FAST_TIMEOUT_SECS, retries=FAST_RETRIES)
    ...
    if status_code == 200:
        ...
        error_msg = api_response.get("error") or str(response_body)
        raise RuntimeError(f"Failed to retrieve User ID for '{user_email}'. API Error: {error_msg}")
...
def add_user_to_group_by_id(group_id, group_full_name, user_email):
    ...
    if not user_id_to_add:
        print(f"Cannot add user. User with email '{user_email}' not found in Databricks workspace.")
        return False
    ...
    api_response = _safe_request("PATCH", url, headers=_auth_headers(), json=payload, timeout=FAST_TIMEOUT_SECS, retries=FAST_RETRIES)
    ...
    if status_code in (200, 204):
        ...
        error_msg = api_response.get("error") or str(api_response.get("body"))
        raise RuntimeError(f"Failed to add user '{user_email}' to group '{group_full_name}'. API Error: {error_msg}")


def create_project_schema(catalog_name: str, schema_name: str, storage_root_url: str) -> Dict[str, Any]:
    """
    ...
    if not all([catalog_name, schema_name, storage_root_url]):
        raise ValueError("Catalog name, schema name, and storage root URL are required for schema creation.")
    ...
    print(f"Creating Unity Catalog Schema: '{full_schema_name}' with storage root: {storage_root_url}...")
    ...
    # Handle the "already exists" case gracefully (Status code 409 Conflict)
        ...
        print(f"WARNING: Unity Catalog Schema '{full_schema_name}' already exists. Skipping creation.")
        return {"name": schema_name, "catalog_name": catalog_name, "status": "exists"}
    
    else:
        error_msg = api_response.get("error") or str(response_body)
        raise RuntimeError(f"Failed to create Unity Catalog Schema '{full_schema_name}'. API Error: {error_msg}")
